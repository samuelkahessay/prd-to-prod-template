...
if [ ! -x "$SCRIPT" ]; then
  echo "RED: $SCRIPT does not exist yet — test defines the degraded contract" >&2
  exit 1
fi
...
echo "gh $*" >> "$CALL_LOG"
case "$*" in
  *"repos/acme/reports --jq .default_branch"*) echo "main" ;;
  *"git/trees/main?recursive=1"*) printf '{"tree":[{"path":"src/reports.ts","type":"blob"}]}\n' ;;
  *"contents/src/reports.ts?ref=main"*) printf '{"content":"ZXhwb3J0IGNvbnN0IHJlcG9ydHMgPSBbXTsK","encoding":"base64"}\n' ;;
esac
STUB
...
printf '%s' "$OUTPUT" | jq -e '.[0].gapAnalysis.currentState == "Analysis failed"' >/dev/null || {
  echo "FAIL: expected degraded fallback gap analysis" >&2
  exit 1
}

echo "analyze-target degraded tests passed"
